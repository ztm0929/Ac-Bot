...
本项目维护者仍在熟悉工程实践，必要时可以比成熟开源项目写更多中文注释。注释应优先解释意图、业务背景、边界条件、风险、约束来源和非显而易见的取舍；不要只复述代码正在做什么，例如“给变量赋值”“遍历数组”。当代码本身已经清楚时，不必为了数量添加注释；当逻辑涉及安全、幂等、平台边界、状态流转或重试策略时，应主动补充简洁注释。
...
新增说明文件、README、用户可见文本、错误说明、代码注释和提交信息时，优先使用中文。专有名词、API 名称、协议字段、代码标识、英文缩写或只用中文难以准确表达的范围、时态、语气和技术含义，可以保留或补充英文。
